...
> - Không stop/restart/recreate `<service-name>` hay bất kỳ service nào khác trong lúc cleanup.
...
0. **Project guard — verify repo identity trước mọi thao tác SSH/cloud/git trên production.**
   ...
   - Sanitize origin về `host/owner/repo` chữ thường: hỗ trợ `git@host:org/repo.git`,
     ...
     dấu `/` đầu, và `.git` cuối.
   - Yêu cầu origin identity đã sanitize khớp CHÍNH XÁC `github.com/acegalaxy-co/ace_commons-ccswitch-cli`. Nếu `github.com/acegalaxy-co/ace_commons-ccswitch-cli`
     có dạng placeholder (`<...>`), origin không parse được, repo-root slug không khớp, hoặc remote identity
     không khớp → STOP ngay; báo user command này thuộc về `ccswitch-cli-claude` / `github.com/acegalaxy-co/ace_commons-ccswitch-cli`,
