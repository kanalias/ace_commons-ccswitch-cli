...
## Host/IP nghi production (lưới cuối, judgment)

Ngoài secret, quét host/IP **nghi là production thật** đã lọt vào file (hook `pre-edit-content-scan.sh` chặn lúc edit, đây là lưới cuối lúc push — bắt cái đã commit từ trước / edit qua Bash lách hook):

1. Grep host trong URL (`https?://<host>`) + bare IPv4 trong phạm vi quét.
2. **Bỏ qua safe-pattern** (như hook): `localhost`, `*.local`, `*.test`, `*.example`, `example.*`, `*.proxy.example.com`, RFC1918 (`10.`, `192.168.`, `172.16-31.`), `127.*`, `169.254.`, `0.0.0.0`, `255.255.255.255`.
3. **Bỏ qua host trong** `.claude/allowed-hosts.txt` (đã user duyệt).
4. Còn lại = nghi prod → **report + HỎI USER** (host này prod thật? cho phép? → thêm vào `.claude/allowed-hosts.txt`). KHÔNG tự quyết là ổn, KHÔNG tự thêm allowlist.
