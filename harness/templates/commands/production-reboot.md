...
Use when `@@DEPLOY_SSH_HOST@@` is **wedged at the userspace layer**: TCP ports accept fast but no
...
0. **Project guard — verify repo identity before any SSH/cloud/git production action.**
   ...
   - If any config used by this command is still placeholder-shaped (`<...>`) — `@@DEPLOY_SSH_HOST@@`, `@@DEPLOY_SERVICE@@` — STOP;
...
4. **Poll SSH until the host is back:**
   ...
   for i in $(seq 1 12); do
     if out=$(ssh -o ConnectTimeout=10 @@DEPLOY_SSH_HOST@@ 'echo SSH_OK; uptime; df -h / | tail -1' 2>/dev/null); then
