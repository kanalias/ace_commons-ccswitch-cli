...
`claude` / `codex` / `deepseek` dùng **CÙNG base URL** `https://proxy.example.com/v1` qua 9router **và CÙNG 1 token** (điền giống nhau vào cả 3 profile); chỉ khác block `ANTHROPIC_DEFAULT_*_MODEL` (prefix `cc/` vs `cx/` vs `ds/`). Vì chung 1 router, router chết = cả 3 chết → fallback duy nhất là `subscription`.
